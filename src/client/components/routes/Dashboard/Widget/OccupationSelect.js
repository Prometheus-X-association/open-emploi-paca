 ...
 *
 */
export function OccupationSelect({className, onSelectOccupationIds = () => []} = {}) {
  ...
  useEffect(() => {
    if (selectedOccupationIds.length === 0 && me?.occupation?.id) {
      selectOccupationIds([me?.occupation?.id, ...me.wishedOccupations.edges.map(({node: occupation}) => occupation.id)]);
    }
  }, [me?.jobArea?.id]);
    ...
    return (
      <ListItem key={occupation.id} className={classes.occupation}>
        <ListItemIcon className={classes.checkbox}>
          <Checkbox size={"small"} edge="start" checked={selectedOccupationIds.includes(occupation.id)}
                    onChange={() => handleSelectOccupation({occupation, index})}/>
        </ListItemIcon>
        <ListItemText primary={occupation.prefLabel} className={classes.occupationText} style={{color: Colors[index]}}/>
      </ListItem>
    );
  }
