...
export default function Market({} = {}) {
  ...
  return (
    <Grid container spacing={3}>
      <Grid item xs={12}>
        <Typography variant={"h2"}>{t("MARKET.TITLE")}</Typography>
      </Grid>
      <Grid item xs={12}>
        <Typography variant={"subtitle1"}>{t("MARKET.TIP_AGGS_OCCUPATIONS")}</Typography>
      </Grid>
            ...
            <If condition={me?.jobArea}>
              <Grid item xs={6}>
                <BlockContainer title={me.jobArea.title} expandable>
                  <OffersByOccupationWidget jobArea={me.jobArea} />
                </BlockContainer>
              </Grid>
            </If>
            ...
            {me.wishedJobAreas.edges.map(({node: jobArea}) => (
              <Grid key={jobArea.id}  item xs={6}>
                <BlockContainer title={jobArea.title} expandable>
                  <OffersByOccupationWidget jobArea={jobArea} />
                </BlockContainer>
              </Grid>
            ))}
          </When>
      ...
      <Grid item xs={12}>
        <Typography variant={"subtitle1"}>{t("MARKET.TIP_AGGS_JOB_AREAS")}</Typography>
      </Grid>
            ...
            {me.wishedOccupations.edges.map(({node: occupation}) => (
              <Grid key={occupation.id} item xs={6}>
                <BlockContainer title={occupation.prefLabel} expandable>
                  <OffersByJobAreaWidget occupation={occupation} />
                </BlockContainer>
              </Grid>
            ))}
          </When>
      ...
      <Grid item xs={12}>
        <Typography variant={"subtitle1"}>{t("MARKET.TIP_AGGS_TOP_ORGANIZATIONS")}</Typography>
      </Grid>
