 ...
 *
 */
export function OccupationDetails({ occupationId, personId } = {}) {
  const classes = useStyles();
  const { t } = useTranslation();
  const { data: { occupation, skills, aptitudes } = {}, loading } = useQuery(
    gqlOccupationDetails,
    {
      variables: {
        occupationId,
        skillsFilters: [`hasOccupationCategory:${occupationId}`],
        aptitudesFilters: [
          `hasRelatedOccupation:${occupationId}`,
          `hasPerson:${personId}`,
        ],
      },
    }
  ...
  const aptitudesLabels = (aptitudes?.edges || []).map(
    ({ node: { skillLabel } }) => skillLabel
  );
  const filteredSkills = (skills?.edges || []).filter(
    ({ node: { prefLabel } }) => !aptitudesLabels.includes(prefLabel)
  );
        ...
        <Otherwise>
          <Grid item container className={classes.titles}>
            <Grid xs item container alignItems="center">
              <Grid xs item>
                <Typography variant={"h6"}>
                  {t("CARTONET.OCCUPATION_MATCHING.SUB_OCCUPATIONS")}
                </Typography>
              </Grid>
              <Grid item>
                <a
            ...
            <Grid xs item container justify={"center"}>
              <Typography variant={"h6"}>
                {t("CARTONET.OCCUPATION_MATCHING.APTITUDES")}
              </Typography>
            </Grid>
            <Grid xs item container justify={"center"}>
              <Typography variant={"h6"}>
                {t("CARTONET.OCCUPATION_MATCHING.OTHER_SKILLS")}
              </Typography>
            </Grid>
          </Grid>
          <Grid xs item container className={classes.details}>
            <Grid xs item className={classes.column}>
              <List dense>
                {(occupation?.relatedOccupationName || []).map(
                  (subOccupation) => (
                    <ListItem key={subOccupation}>
                      <ListItemText primary={subOccupation} />
                    </ListItem>
                  )
                )}
              </List>
              ...
              <Choose>
                ...
                <When condition={(aptitudes?.edges || []).length === 0}>
                  <div className={classes.empty}>
                    {t("CARTONET.OCCUPATION_MATCHING.NO_APTITUDE")}
                  </div>
                </When>
                <Otherwise>
                  <List dense>
            ...
            <Grid xs item className={classes.column}>
              <Choose>
                <When condition={(filteredSkills || []).length === 0}>
                  <div className={classes.empty}>
                    {t("CARTONET.OCCUPATION_MATCHING.NO_SKILL")}
                  </div>
                </When>
                <Otherwise>
                  <List dense>
