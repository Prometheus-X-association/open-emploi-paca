...
export function dseIdToURI({id, modelDefinition, synaptixSession}) {
  ...
  let uri = synaptixSession.generateUriForModelDefinition({modelDefinition, idValue});
  return uri.replace(synaptixSession.getNodesNamespaceURI(), `${synaptixSession.getNodesPrefix()}:`)
}